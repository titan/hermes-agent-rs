...
pub async fn get_config(State(state): State<HttpServerState>) -> Json<serde_json::Value> {
...
pub async fn get_defaults(State(state): State<HttpServerState>) -> Json<serde_json::Value> {
    let _ = &state;
    let defaults =
        serde_json::to_value(hermes_config::GatewayConfig::default()).unwrap_or_default();
    Json(defaults)
}

/// GET /api/config/schema — return config field schema for the form editor.
pub async fn get_schema(State(state): State<HttpServerState>) -> Json<serde_json::Value> {
    let _ = &state;
...
pub async fn update_config(
    ...
    let yaml = serde_yaml::to_string(&body.config).map_err(|e| HttpError {
        status: axum::http::StatusCode::BAD_REQUEST,
        message: format!("invalid config: {}", e),
    })?;
    std::fs::write(&config_path, yaml).map_err(|e| HttpError {
...
pub async fn update_config_raw(
    ...
    let _: serde_yaml::Value = serde_yaml::from_str(&body.yaml_text).map_err(|e| HttpError {
        status: axum::http::StatusCode::BAD_REQUEST,
        message: format!("invalid YAML: {}", e),
    })?;

    let config_path = state.hermes_home.join("config.yaml");
    std::fs::write(&config_path, &body.yaml_text).map_err(|e| HttpError {
        status: axum::http::StatusCode::INTERNAL_SERVER_ERROR,
        message: format!("failed to write config: {}", e),
    })?;
    Ok(Json(OkResponse { ok: true }))
...
pub async fn get_model_info(State(state): State<HttpServerState>) -> Json<ModelInfoResponse> {
    let model = state
        .config
        .model
        .clone()
