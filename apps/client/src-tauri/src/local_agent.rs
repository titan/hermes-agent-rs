...
{
    let config =
        hermes_config::load_config(None).map_err(|e| format!("Failed to load config: {}", e))?;
    ...
    eprintln!(
        "[local-agent] model={}, provider_count={}",
        effective_model,
        config.llm_providers.len()
    );
    for (name, cfg) in &config.llm_providers {
        eprintln!(
            "[local-agent] provider '{}': has_key={}, base_url={:?}",
            name,
            cfg.api_key.is_some(),
            cfg.base_url
        );
    ...
    eprintln!(
        "[local-agent] agent done, {} messages returned",
        result.messages.len()
    );
